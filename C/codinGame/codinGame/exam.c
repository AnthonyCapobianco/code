...
#define iIs "%i est %s et la somme de ses diviseurs est de: %i\n",i,
#define ASK "Veillez introduire un nombre entier positif comme borne superieur: "
int main(void)
{
    int   max = 0
         ,sum = 0
    ...
    do
    {
        printf(ASK);
        v = scanf("%i", &max);
        if(i > 0) printf("Le nombre ne peut pas être inférieur à 2\n");
        ++i;
    }
    while (max < 2);
    ...
    if(v)
    {
        for(i = 2; i <= max; ++i)
        {
            ...
            if (sum == i && uP) printf(iIs par, sum);
            else if (sum < i && uD) printf(iIs def, sum);
            else if (sum > i && uA) printf(iIs abo, sum);
        }// for(i <= max)
    }// if(v)
    return 0;
}
